...
import { resetSessionRedirectUrl } from './sessionRedirectUrl';
...
interface JwtPayload {
  ...
  [key: string]: any; // Additional properties depending on your token
}
...
const defaultAuthContext: JwtAuthContextType = {
  ...
  user: undefined,
  updateUser: () => { },
  signIn: async () => new AxiosError('Sign in failed, please try again!'),
  signUp: async () => new AxiosError('Sign up failed, please try again!'),
  signOut: () => { },
  refreshToken: async () => { },
  setIsLoading: () => { },
...
function JwtAuthProvider(props: JwtAuthProviderProps) {
  ...
  const handleError = useCallback((error: AxiosError) => {
    // dispatch(showMessage({ message: error?.response?.data ? error.response.data : 'Đăng nhập thất bại!!!' },));
    resetSession();
    setIsAuthenticated(false);
    setUser(null);
  }, []);
  ...
  useEffect(() => {
    const attemptAutoLogin = async () => {
      ...
      if (accessToken && isTokenValid(accessToken)) {
        try {
          setIsLoading(true);
          // const response: AxiosResponse<User> = await axios.post(config.getUserUrl, {}, {
          //   headers: { Authorization: `Bearer ${accessToken}` }
          // });
          const response: AxiosResponse<User> = await axios.get(config.getUserUrl, { headers: { Authorization: `Bearer ${accessToken}` } })
